  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', height: '100%' }}>
        ...
        <AnimatePresence mode="wait">
          {selected ? (
            <motion.div
              ...
              className="email-desktop-detail"
          ...
          >
            ...
            <div style={{ padding: '20px 16px' }}>
              <EmailDetail
                ...
                onReply={() => openReply(selected)}
                onStar={() => toggleStar(selected.id)}
                onDelete={() => { deleteEmail(selected.id); closeMobileDetail(); }}
                isMobile
              />
            </div>
          </motion.div>
      ...
      <Modal open={composeOpen} onClose={() => setComposeOpen(false)} title={replyTo ? `Reply to ${replyTo.sender}` : 'New Email'}
        footer={<><Btn variant="ghost" onClick={() => setComposeOpen(false)}>Cancel</Btn><Btn variant="primary" onClick={handleSend}>Send</Btn></>}>
        ...
        <div style={{ fontSize: '11px', color: 'var(--text3)', marginTop: '4px' }}>Connect an SMTP provider to deliver real emails.</div>
      </Modal>

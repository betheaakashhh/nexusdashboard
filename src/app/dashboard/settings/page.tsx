...
export default function SettingsPage() {
  ...
  // Forgot PIN — verify account password to reset
  async function handleForgotPin() {
    ...
    try {
      ...
      // Step 2: delete old PIN (backend will allow since we have a special bypass endpoint)
      // We use a special reset endpoint — send account password as proof
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', height: '100%' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        {/* Left nav */}
        <div style={{ width: '200px', minWidth: '200px', background: 'var(--bg2)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }} className="settings-sidenav">
            ...
            <motion.div key={section} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }}>
              ...
              {section === 'appearance' && (
                <div style={{ maxWidth: '580px' }}>
                  ...
                  <SettingRow label="Color scheme" description="Choose how Nexus looks. Changes apply instantly.">
                    <PillGroup
                      options={[
                  ...
                  <div style={{ marginTop: '24px' }}>
                    ...
                    <p style={{ fontSize: '11.5px', color: 'var(--text3)', marginBottom: '12px' }}>
                      Changes buttons, active states, and highlights throughout Nexus.
                    </p>
                      ...
                      <label title="Custom color" style={{
                        ...
                        <input type="color" value={settings.accentColor}
                          onChange={(e) => {
                            ...
                            const next = { ...settings, accentColor: e.target.value };
                    ...
                    <div style={{ marginTop: '12px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <span style={{ fontSize: '11px', color: 'var(--text3)', padding: '2px 8px', background: 'var(--bg3)', borderRadius: 4, border: '1px solid var(--border)' }}>
                        Live preview active
                      </span>
                    </div>
                  </div>
              ...
              {section === 'security' && (
                <div style={{ maxWidth: '560px' }}>
                  ...
                  <div style={{ marginTop: '24px' }}>
                    ...
                    <SettingRow label="Auto-logout" description="Sign out after inactivity (applied on next load)">
                      <PillGroup
                        options={[
                          { value: 0 as 0,   label: 'Off'   },
                          { value: 5 as 5,   label: '5 min' },
                          { value: 15 as 15, label: '15m'   },
                          { value: 30 as 30, label: '30m'   },
                          { value: 60 as 60, label: '1 hr'  },
                        ]}
                  ...
                  <div style={{ marginTop: '24px' }}>
                    ...
                    <div style={{
                      padding: '16px', background: 'var(--bg3)', borderRadius: 'var(--r)',
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '11.5px', color: 'var(--text3)', marginTop: '2px' }}>
                            {hasPin
                              ? 'Your vault requires a PIN before opening. PIN is stored as a secure hash in the database — never in your browser.'
                              : 'Add a PIN to require extra authentication before opening Private Vault. Stored securely in the database.'}
                          </div>
                        </div>
                      </div>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--text3)', display: 'flex', alignItems: 'center', gap: '5px' }}>
                      ...
                      PIN is hashed with bcrypt and stored in your database, not in cookies or localStorage.
                    </div>
                  </div>
                ...
                <div style={{ maxWidth: '560px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
                    <FormField label="Default sender name">
                      <Input value={settings.defaultSenderName} onChange={(e) => saveSettings({ defaultSenderName: e.target.value })}
                        onBlur={(e) => save({ defaultSenderName: e.target.value })} placeholder="Your Name" />
                    </FormField>
                    <FormField label="Default from email">
                      <Input type="email" value={settings.defaultFromEmail}
                        onChange={(e) => saveSettings({ defaultFromEmail: e.target.value })}
                        onBlur={(e) => save({ defaultFromEmail: e.target.value })}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '10px' }}>
                      {settings.quickRecipients.length === 0 ? (
                        ...
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 12px', background: 'var(--bg3)', borderRadius: 'var(--r2)', border: '1px solid var(--border)' }}>
                          ...
                          <button onClick={() => save({ quickRecipients: settings.quickRecipients.filter((_, j) => j !== i) })}
      ...
      <Modal open={pinOpen} onClose={() => setPinOpen(false)} title={hasPin ? 'Change Vault PIN' : 'Set Vault PIN'}
        ...
        {hasPin && (
          <FormField label="Current PIN">
            <Input type="password" inputMode="numeric" value={pinForm.current} onChange={(e) => setPinForm({ ...pinForm, current: e.target.value.replace(/\D/g, '') })} placeholder="Enter current PIN" />
          </FormField>
        )}
      ...
      <Modal open={removePin} onClose={() => setRemovePin(false)} title="Remove Vault PIN"
        ...
        <div style={{ fontSize: '13px', color: 'var(--text2)', marginBottom: '16px' }}>
          Enter your current PIN to confirm removal. After this, Private Vault will open without a PIN.
        </div>
        <FormField label="Current PIN">
          <Input type="password" inputMode="numeric" value={pinForm.current} onChange={(e) => setPinForm({ ...pinForm, current: e.target.value.replace(/\D/g, '') })} placeholder="••••" />
        </FormField>
      </Modal>
      ...
      <Modal open={forgotPin} onClose={() => setForgotPin(false)} title="Reset Vault PIN"
        ...
        <div style={{ fontSize: '12.5px', color: 'var(--text3)', marginBottom: '16px', padding: '10px 12px', background: 'var(--bg3)', borderRadius: 'var(--r2)', border: '1px solid var(--border)' }}>
          To prove it's you, enter your <strong style={{ color: 'var(--text)' }}>account password</strong> (not PIN). Then choose a new PIN.
        </div>
        <FormField label="Account password (to verify identity)">
          <Input type="password" value={passwordForPinReset} onChange={(e) => setPasswordForPinReset(e.target.value)} placeholder="Your login password" />
        </FormField>
        <FormField label="New PIN (min. 4 digits)">
          <Input type="password" inputMode="numeric" value={pinForm.pin} onChange={(e) => setPinForm({ ...pinForm, pin: e.target.value.replace(/\D/g, '') })} placeholder="••••" />
        </FormField>
